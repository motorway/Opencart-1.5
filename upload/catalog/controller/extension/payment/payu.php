...
class ControllerExtensionPaymentPayu extends Controller
{
	public function index()
	{
		...
		if (intval($this->config->get('payu_entry_order_type') == 0)) {
			$vat_type = 'NET';
		} else {
			$vat_type = 'GROSS';
		}
		...
		if ($this->config->get('payu_backref') != "") {
			$forSend['BACK_REF'] = $this->config->get('payu_backref');
		} else {
			$forSend['BACK_REF'] = $this->url->link('extension/payment/payu/result_payment');
		}
		...
		foreach ($pref as $k => $v) {
			...
			if (isset($order_info[$bill])) {
				$forSend["BILL_".$k] = $order_info[$bill];
			}

			if (isset($order_info[$deliv])) {
				$forSend["DELIVERY_".$k] = $order_info[$deliv];
			}
		}
	...
	public function confirm()
	{
		if ($this->session->data['payment_method']['code'] == 'payu') {
			// $this->load->model('checkout/order');
			// $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payu_order_status_id'));
		}
	}
	...
	public function callback()
	{
		...
		$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payu_order_status_id'));
